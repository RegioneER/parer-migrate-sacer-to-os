...
import static it.eng.parer.migrate.sacer.os.base.utils.Costants.S3_KEY_URN_FIRMA_ELENCO_INDICI_AIP_FMT;
import static it.eng.parer.migrate.sacer.os.base.utils.Costants.S3_KEY_URN_MARCA_FIRMA_ELENCO_INDICI_AIP_FMT;
...
@ApplicationScoped
public class MigrateOsElencoVersAipS3Service extends MigrateOsS3Abstract
	implements IMigrateOsElencoVersAipS3Service {
	...
	try {
	    ElvFileElencoVers elvFileElencoVers = sacerElencoVersAipDao
		...
		sacerElencoVersAipDao.deleteBlFileElencoVers(idFileElencoVers);
	    }
	...
	String urn = calculateUrnFileElencoIndiciAip(elvElencoVers, elvFileElencoVers);
	return createResourcesElencoIndiciAip(urn, elvFileElencoVers, idSacerBackend);
    }

    /*
     * Calcolo dell'URN dei file dell'elenco di versamento normalizzato
     */
    private String calculateUrnFileElencoIndiciAip(ElvElencoVers elvElencoVers,
	...
	case FIRMA_ELENCO_INDICI_AIP:
	    format = S3_KEY_URN_FIRMA_ELENCO_INDICI_AIP_FMT;
	    break;
	case MARCA_FIRMA_ELENCO_INDICI_AIP:
	    format = S3_KEY_URN_MARCA_FIRMA_ELENCO_INDICI_AIP_FMT;
	    break;
	default:
	    throw new IllegalArgumentException(
		    "Unsupported file type: " + elvFileElencoVers.getTiFileElencoVers());
	}
	String tmpUrn = formattaBaseUrnElencoVersAip(
		formatDateAsStr(elvElencoVers.getDtCreazioneElenco()),
		elvElencoVers.getIdElencoVers().toString(), format);
	...
	try {
	    ...
	    return formattaBaseUrnElencoVersAip(urn_versatore, urnIndiceElencoVers, fmt);
	} catch (Exception e) {
	    throw AppMigrateOsS3Exception.builder().category(ErrorCategory.S3_ERROR).cause(e)
		    .message(
			    "Errore durante calcolo URN con formato {0} e valori nmEnte {1} / nmStrut {2}",
			    fmt, nmEnte, nmStrut)
		    .build();
	}
    ...
    private static String formattaBaseUrnElencoVersAip(String versatore, String indiceElencoVers,
	    String fmtUsed) {
	return MessageFormat.format(fmtUsed, versatore, indiceElencoVers);
    }

    private IObjectStorageResource createResourcesElencoIndiciAip(final String urn,
	...
	try {
	    IObjectStorageResource osresource = createElencoVersAipAndPutOnBucket(urn, filepath,
		    elvFileElencoVers);
	    // link
	    sacerElencoVersAipDao.saveObjectStorageLinkElencoIndiciAipUd(osresource.getTenant(),
    ...
    private IObjectStorageResource createElencoVersAipAndPutOnBucket(final String urn,
