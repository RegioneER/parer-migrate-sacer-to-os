...
class MigrateOsElencoVersAipServiceTest {
    @Inject
    IMigrateOsElencoVersAipService service;
	...
	List<RequestDto> dtos = assertDoesNotThrow(() -> service
		.registerMigrationIndiceAipElencoVersUdRequest(Arrays.asList(request)));
	...
	List<RequestDto> dto1 = assertDoesNotThrow(() -> service
		.registerMigrationIndiceAipElencoVersUdRequest(Arrays.asList(request1)));
	...
	List<RequestDto> dto2 = assertDoesNotThrow(() -> service
		.registerMigrationIndiceAipElencoVersUdRequest(Arrays.asList(request2)));
	...
	assertDoesNotThrow(() -> service.processMigrationIndiceAipElencoVersUdFromRequest(1L));
	...
	assertThrows(AppGenericRuntimeException.class,
		() -> service.processMigrationIndiceAipElencoVersUdFromRequest(null));
	...
	assertThrows(AppBadRequestException.class, () -> service
		.registerMigrationIndiceAipElencoVersUdRequest(Arrays.asList(request1, request2)));
    }
	...
	List<RequestDto> dtos = assertDoesNotThrow(
		() -> service.registerMigrationIndiceAipElencoVersUdRequest(Arrays.asList()));
	assertTrue(dtos.isEmpty());
    }
}
