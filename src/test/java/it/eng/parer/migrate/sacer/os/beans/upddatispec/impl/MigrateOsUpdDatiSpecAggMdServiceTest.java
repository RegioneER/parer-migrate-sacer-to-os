...
class MigrateOsUpdDatiSpecAggMdServiceTest {
    @Inject
    IMigrateOsUpdDatiSpecAggMdService service;
	...
	List<RequestDto> dtos = assertDoesNotThrow(
		() -> service.registerMigrationUpdDatiSpecAggMdRequest(Arrays.asList(request)));
	...
	List<RequestDto> dto1 = assertDoesNotThrow(
		() -> service.registerMigrationUpdDatiSpecAggMdRequest(Arrays.asList(request1)));
	...
	List<RequestDto> dto2 = assertDoesNotThrow(
		() -> service.registerMigrationUpdDatiSpecAggMdRequest(Arrays.asList(request2)));
	...
	assertDoesNotThrow(() -> service.processMigrationUpdDatiSpecAggMdFromRequest(1L));
	...
	assertThrows(AppGenericRuntimeException.class,
		() -> service.processMigrationUpdDatiSpecAggMdFromRequest(null));
	...
	assertThrows(AppBadRequestException.class, () -> service
		.registerMigrationUpdDatiSpecAggMdRequest(Arrays.asList(request1, request2)));
    }
	...
	List<RequestDto> dtos = assertDoesNotThrow(
		() -> service.registerMigrationUpdDatiSpecAggMdRequest(Arrays.asList()));
	assertTrue(dtos.isEmpty());
    }
}
