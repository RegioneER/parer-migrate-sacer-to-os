...
class MigrateOsCompServiceTest {

    @Inject
    IMigrateOsCompService service;
	...
	List<RequestDto> dtos = assertDoesNotThrow(
		() -> service.registerMigrationCompRequest(Arrays.asList(request)));
	...
	List<RequestDto> dto1 = assertDoesNotThrow(
		() -> service.registerMigrationCompRequest(Arrays.asList(request1)));
	...
	List<RequestDto> dto2 = assertDoesNotThrow(
		() -> service.registerMigrationCompRequest(Arrays.asList(request2)));
	...
	assertDoesNotThrow(() -> service.processMigrationCompFromRequest(1L));
	...
	assertThrows(AppGenericRuntimeException.class,
		() -> service.processMigrationCompFromRequest(null));
	...
	assertThrows(AppBadRequestException.class,
		() -> service.registerMigrationCompRequest(Arrays.asList(request1, request2)));
    }
	...
	List<RequestDto> dtos = assertDoesNotThrow(
		() -> service.registerMigrationCompRequest(Arrays.asList()));
	assertTrue(dtos.isEmpty());
    }
