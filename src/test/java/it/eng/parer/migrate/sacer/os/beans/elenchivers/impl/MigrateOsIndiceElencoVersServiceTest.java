...
class MigrateOsIndiceElencoVersServiceTest {
    @Inject
    IMigrateOsIndiceElencoVersService service;
	...
	List<RequestDto> dtos = assertDoesNotThrow(
		() -> service.registerMigrationIndiceElencoVersUdRequest(Arrays.asList(request)));
	...
	List<RequestDto> dto1 = assertDoesNotThrow(
		() -> service.registerMigrationIndiceElencoVersUdRequest(Arrays.asList(request1)));
	...
	List<RequestDto> dto2 = assertDoesNotThrow(
		() -> service.registerMigrationIndiceElencoVersUdRequest(Arrays.asList(request2)));
	...
	assertDoesNotThrow(() -> service.processMigrationIndiceElencoVersUdFromRequest(1L));
	...
	assertThrows(AppGenericRuntimeException.class,
		() -> service.processMigrationIndiceElencoVersUdFromRequest(null));
	...
	assertThrows(AppBadRequestException.class, () -> service
		.registerMigrationIndiceElencoVersUdRequest(Arrays.asList(request1, request2)));
    }
	...
	List<RequestDto> dtos = assertDoesNotThrow(
		() -> service.registerMigrationIndiceElencoVersUdRequest(Arrays.asList()));
	assertTrue(dtos.isEmpty());
    }
}
