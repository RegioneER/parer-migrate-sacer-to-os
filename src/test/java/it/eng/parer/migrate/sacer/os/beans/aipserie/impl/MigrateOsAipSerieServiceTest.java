...
class MigrateOsAipSerieServiceTest {
    @Inject
    IMigrateOsAipSerieService service;
	...
	List<RequestDto> dtos = assertDoesNotThrow(
		() -> service.registerMigrationAipSerieRequest(Arrays.asList(request)));
	...
	List<RequestDto> dto1 = assertDoesNotThrow(
		() -> service.registerMigrationAipSerieRequest(Arrays.asList(request1)));
	...
	List<RequestDto> dto2 = assertDoesNotThrow(
		() -> service.registerMigrationAipSerieRequest(Arrays.asList(request2)));
	...
	assertDoesNotThrow(() -> service.processMigrationAipSerieFromRequest(1L));
	...
	assertThrows(AppGenericRuntimeException.class,
		() -> service.processMigrationAipSerieFromRequest(null));
	...
	List<RequestDto> dtos = assertDoesNotThrow(
		() -> service.registerMigrationAipSerieRequest(Arrays.asList()));
	assertTrue(dtos.isEmpty());
    }
	...
	assertThrows(AppBadRequestException.class,
		() -> service.registerMigrationAipSerieRequest(Arrays.asList(request1, request2)));
    }
